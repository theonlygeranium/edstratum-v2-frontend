...
- GitHub Actions Node 24 migration commit `d01ce68` is deployed on production. It updates action pins to `actions/checkout@v5`, `actions/setup-node@v5`, `actions/upload-artifact@v6`, and `actions/github-script@v8` while leaving the app build runtime at `node-version: '20'`. Hosted main CI `29739390956` passed with `156 passed`; strict log search found no deprecated Node.js 20 JavaScript-action warning. Live `/build-manifest.json` returned commit `d01ce68` with unchanged code-bearing assets: entry `/assets/index-Cld5-OrE.js`, stylesheet `/assets/index-DH0EGGDC.css`, chat asset `/assets/StratumChat-5iN0axbq.js`, and 13 total assets.
...
- Frontend CI action pins are now Node 24-native at commit `d01ce68`. The workflow app runtime still requests `node-version: '20'`; moving the app runtime to Node 24 is a separate future change and should include a normal full frontend QA pass.
...
12. Evaluate a separate frontend CI app-runtime move from Node 20 to Node 24, or pin Wrangler to a Node 20-compatible version, because the action-runtime warning fix did not change the app runtime.
