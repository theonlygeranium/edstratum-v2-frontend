...
- GitHub Actions action-migration commit verified: `d01ce68`; frontend CI app-runtime migration commit verified: `f2c969b`; CI Playwright server-ownership fix commit `84e01ce` is already contained in current `main`; Wrangler pin commit `76b97ba`, live-smoke command commit `bb8f3b4`, and rendered live-smoke command commit `52cdf47` are deployed, but hosted CI proof is pending because GitHub Actions run `29744827315` for report commit `91f2ff1` failed before starting any steps due to an account billing/spending-limit blocker.
...
- Backend release/governance audit command commit `d45f4c9` is deployed on Railway. It adds `scripts/live_release_audit.py` in the backend repo for non-mutating GitHub, Cloudflare, Railway, and public runtime audit coverage. The full audit currently exits nonzero with four expected governance blockers: frontend branch protection, backend branch protection, frontend Actions billing failure, and backend Actions billing failure. Public runtime checks inside the audit pass.
...
- Cross-surface release governance now has a backend audit: `.venv/bin/python scripts/live_release_audit.py` from the backend repo. It is non-mutating and exits nonzero while branch protection or hosted CI blockers remain.
...
- GitHub Actions currently has an account billing/spending-limit blocker. Frontend run `29744827315` for commit `91f2ff1` and backend run `29745225484` for commit `d45f4c9` failed before starting any workflow steps. Hosted CI proof for later pushes is pending until billing/settings are corrected and the workflows are rerun.
...
7. Resolve the GitHub Actions account billing/spending-limit blocker, then rerun `CI / build-and-test` for `91f2ff1` or the latest frontend commit.
...
12. Keep `EXPECTED_MANIFEST_COMMIT=<short-sha> npm run qa:live` as the first source-controlled deploy verification check on future frontend pushes, then run `EXPECTED_MANIFEST_COMMIT=<short-sha> npm run qa:live:rendered` when rendered production proof is needed.
